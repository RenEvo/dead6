...
namespace stl
{
	...
	public:
		...
		static void* AtomicCompareAndSwapPtr(void* volatile* dst, void* val, void* comp)
		{
			...
			// Since there is currently no CryInterlockedCompareExchangePtr() we will have to use locks for 64-bit
			// environments.
			void* res = *dst;
			if (comp == res)
				*dst = val;
			return res;
#elif defined(_WIN32) || defined(WIN32) || defined(XENON) || defined(PS3)
			//if (!((long)val == 0 || (long)val > 10000))
			//	_asm { int 3 };
			return (void*)CryInterlockedCompareExchange((long volatile*)dst, (long)val, (long)comp);
#else
			// Since there is currently no CryInterlockedCompareExchangePtr() we will have to use locks for non-windows
			// environments.
			void* res = *dst;
