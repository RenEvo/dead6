...
void CHUDScore::Render()
{
	...
	if(teamCount > 1)
	{
		...
		if(currentRank)
		{
			drawTeamScores=false; // if we got here it means we don't want to show the number of kills

			const char* nextRankName = 0;
			const char* currentRankName = 0;
			int currentRankCP = 0;
			int nextRankCP = 0;
			int currentRankPP = 0;
			int nextRankPP = 0;
			...
			if(nextRankName)
			{
				...
				if(pGameRulesScript->GetValue("GetRankCP", pfnFuHelper) && pfnFuHelper)
				{
					...
					m_pFlashBoard->Invoke("setClientRankProgress", args, 4);
				}
				...
				if(pGameRulesScript->GetValue("GetRankPP", pfnFuHelper) && pfnFuHelper)
				{
					...
					m_pFlashBoard->Invoke("setMinimumPrestige", args, 2);
				}
			}
		}
	}
