...
gSkip[] = {
	...
	{CFlashMenuObject::eEMS_Rating,1},
...
#if defined(PS3)
	//somehow we have to apply this constant to x and y to convert screen to flash coordinates
//	static const float g_XYConvMagic = 1280.f / 1366.f;
static const float g_XYConvMagic = 1.f;
#endif
...
SFlashKeyEvent CFlashMenuObject::MapToFlashKeyEvent(const SInputEvent &inputEvent)
{
	...
	if (inputEvent.state == eIS_UI)
	{
		keyCode = SFlashKeyEvent::A; // !!! Hack !!! We should properly map key code here, too!
...
bool CFlashMenuObject::OnInputEvent(const SInputEvent &rInputEvent)
{
	...
	if(m_bLoadingDone)
	{
		if(g_pGame->GetIGameFramework())
		{
			if((rInputEvent.deviceId == eDI_Mouse || rInputEvent.deviceId == eDI_XI //GC2007 : press any button/key
				|| rInputEvent.deviceId == eDI_Keyboard) && rInputEvent.state == eIS_Pressed)
			{
  ...
  if(m_pCurrentFlashMenuScreen && m_bCatchNextInput && !gEnv->pConsole->GetStatus())
	...
	Vec2 dirvec(0,0);
	if(rInputEvent.keyId == eKI_Up || rInputEvent.keyId == eKI_XI_DPadUp || rInputEvent.keyId == eKI_PS3_Up)
	{
		move = true;
		direction = "up";
		dirvec = Vec2(0,-1);
	}
	else if(rInputEvent.keyId == eKI_Down || rInputEvent.keyId == eKI_XI_DPadDown || rInputEvent.keyId == eKI_PS3_Down)
	{
		move = true;
		direction = "down";
		dirvec = Vec2(0,1);
	}
	else if(rInputEvent.keyId == eKI_Left || rInputEvent.keyId == eKI_XI_DPadLeft || rInputEvent.keyId == eKI_PS3_Left)
	{
		move = true;
		direction = "left";
		dirvec = Vec2(-1,0);
	}
	else if(rInputEvent.keyId == eKI_Right || rInputEvent.keyId == eKI_XI_DPadRight || rInputEvent.keyId == eKI_PS3_Right)
	{
		move = true;
		direction = "right";
		dirvec = Vec2(1,0);
...
#else
			OnHardwareMouseEvent((int)m_CurCursorPos.x, (int)m_CurCursorPos.y, rInputEvent.state == eIS_Pressed ? HARDWAREMOUSEEVENT_LBUTTONDOWN : HARDWAREMOUSEEVENT_LBUTTONUP);
#endif
		}
		//	Navigate
...
void CFlashMenuObject::OnLoadingComplete(ILevel *pLevel)
{
	...
	if(gEnv->bMultiplayer && !(m_multiplayerMenu && m_multiplayerMenu->GetProfile() && m_multiplayerMenu->GetProfile()->IsLoggedIn()))
	{
		...
		if(pProfileMan)
		{
			...
			if(pProfile)
			{
				if(stricmp(pProfile->GetName(),"default"))
				{
					string command("name ");
					command.append(pProfile->GetName());
					command.append(" s");
...
#if defined(PS3)
			//force initialization of first button in next menu
			m_InitButton = false;
#endif
		}
...
{
#if defined(PS3)
	//update button snap does not work properly without hardware mouse, FIXME
	return;
#else
	Vec2 mouseNew(mouse);
...
#if defined(PS3)
	//since there is no hardware mouse available, apply a safe next button snap here
	//right key and down key snap to the next button, the others to the previous one
	//this must get improved obviously, but this hack here makes us at least reach all buttons
	if(m_currentButtons.size() > 1)
	{
		const bool cNext = (dir.x > 0.f || dir.y > 0.f);//true = go to next button
		if(cNext)
		{
			++current;
...
#if defined(PS3)
	m_CurCursorPos.x = pos.x * g_XYConvMagic;
	m_CurCursorPos.y = pos.y * g_XYConvMagic;
#endif
}
...
#else
	OnHardwareMouseEvent((int)m_CurCursorPos.x, (int)m_CurCursorPos.y, HARDWAREMOUSEEVENT_MOVE);
#endif
}
...
#else
		OnHardwareMouseEvent((int)m_CurCursorPos.x, (int)m_CurCursorPos.y, press ? HARDWAREMOUSEEVENT_LBUTTONDOWN : HARDWAREMOUSEEVENT_LBUTTONUP);
#endif
	}
	else if (!press)
	{
...
void CFlashMenuObject::HandleFSCommand(const char *szCommand,const char *szArgs)
{
  if(g_pGameCVars->g_debug_fscommand)
	...
	else if(!strcmp(szCommand, "FlashGetKeyFocus"))
	{
		...
		if(m_pCurrentFlashMenuScreen && m_iGamepadsConnected>0)
		{
...
#if defined(PS3)
		//must get improved, highlight the first button
		if(!m_InitButton)
		{
			Vec2 pos;
			GetButtonClientPos(m_currentButtons.begin(), pos);
			OnHardwareMouseEvent((int)m_CurCursorPos.x, (int)m_CurCursorPos.y, HARDWAREMOUSEEVENT_MOVE);
			m_InitButton = true;
		}
#endif
...
*/
	...
	else if(!strcmp(szCommand,"credits_start"))
	{
		...
		if(m_pMusicSystem)
		{
			...
			m_pMusicSystem->SetTheme("airfield_battle", true, false);
			m_pMusicSystem->SetMood("middle", true, true);
		}	
	}
