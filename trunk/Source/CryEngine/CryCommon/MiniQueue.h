...
public:
	...
	void Erase( SIterator where )
	{
		...
		for (size_t offset = where.Offset(); offset < m_nOffset + m_nValues - 1; offset ++)
		{
  ...
  void Erase( SIterator first, SIterator last )
  {
    ...
    for (size_t offset = first.Offset(); offset < m_nOffset + m_nValues - 1; offset++)
    {
